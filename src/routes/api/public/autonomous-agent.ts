...
Respond with ONLY valid JSON — no prose, no markdown fences:
{
  ...
  "trades": [ { "symbol": "NVDA", "direction": "long|short", "instrument": "${ALL_PROPOSABLE_INSTRUMENT_TYPES.join("|")}", "conviction": <0-100>, "allocation_pct": <1-${effectiveMaxPositionPct}>, "stop_loss_pct": <number>, "take_profit_pct": <number>, "hold_duration": "intraday|swing|position", "rationale": "2-3 sentence explanation", "options_details": { "expiry_days_out": 21, "strike_type": "atm|otm_1|otm_2|itm_1", "contracts": 1, "spread_width": null } } ],
  ...
  for (const raw of ai.trades ?? []) {
    ...
    const isOptionsInstrument = isOptionsInstrumentType(t.instrument);
