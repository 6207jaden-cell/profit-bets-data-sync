-- Adds the Experiment 1 (Claude Value Test) resolution cron: runs once
-- daily, resolves shadow_candidate_log rows past their session-appropriate
-- horizon against real or hypothetical outcomes. Not time-critical (unlike
-- the trading crons), so once a day is sufficient. Every other job is
