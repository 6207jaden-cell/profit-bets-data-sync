-- Adds a daily cron calling rate_limit_cleanup() (defined in the rate
-- limiting migration) so the rate_limit_state table doesn't grow
-- unbounded. Every other job preserved exactly as currently registered.
--
-- Also registers 'evaluate-alerts' as an actual cron job — discovered
-- while working on Priority 3 that this endpoint's own code comment
-- claims it's "called by pg_cron every 5 minutes" but it was never
-- actually present in register_all_crons()'s job list. This means the
-- price-alert-checking feature has likely never been running in
-- production. Noted here rather than silently fixed without mention.
